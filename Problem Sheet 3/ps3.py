...
def deal_hand(n):
    ...
    hand={}
    # Add a wildcard to every hand dealt
    hand['*'] = 1

    num_vowels = int(math.ceil(n / 3)-1) # -1 to account for one wildcard
...
def calculate_handlen(hand):
    ...
    returns: integer
    """
    # Initialise length variable
    length = 0
    # Add up all the amount of letters to length variable
    for letter in hand:
        length += hand[letter]
    # Return number of letters in hand
    return length
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand) > 0:
        ...
        display_hand(hand)
        # Ask user for input
        user_input = input('Enter your word, or "!!" to indicate that you are finished: ')
...
def substitute_hand(hand, letter):
    ...
    if letter not in hand:
        return hand
    # Initialise substitute hand
    sub_hand = hand.copy()
    # Create a list copy of vowels without letters from hand
    sub_vowels = VOWELS
    # Create a string copy of consonants without letters from hand
    sub_consonants = CONSONANTS
    for hand_letter in hand.keys():
        if hand_letter in VOWELS:
            sub_vowels.replace(hand_letter, "")
        elif hand_letter in CONSONANTS:
            sub_consonants.replace(hand_letter, "")
    # Delete letter from subhand
    del(sub_hand[letter])
    # If letter is a vowel, replace letter with random vowel
    if letter in VOWELS:
        sub_hand[random.choice(VOWELS)] = hand[letter]
    # Else letter is a consonant, replace letter with random vowel
    else:
        sub_hand[random.choice(CONSONANTS)] = hand[letter]
    # Return substitute hand
    return sub_hand
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    scores = []
    # Initialise replay bool
    replay = True
    # Initialise total score count
    total_score = 0
    # Intialise best hand:
    best_hand = 0
    # Initialise if letter has been substituted
    letter_sub = False
    # Initialise current hand score
    curr_hand_score = 0
    # Ask user for input of number of hands
    hand_number = int(input("Enter total number of hands: "))
    # Loop until no more hands wanted
    while hand_number > 0:
        # Deal first hand and display to user
        hand = deal_hand(HAND_SIZE)
        # Ask if they would like a substitution
        if not letter_sub:
            print("Current hand: ", end="")
            display_hand(hand)
            user_sub = input("Would you like to substitute a letter? ")
        # If user wants a substitution
            if user_sub.lower() == 'yes' or user_sub.lower() == 'yeah' or user_sub.lower() == 'yep' or user_sub.lower() == 'y':
                letter_sub = True
                sub_letter = input("Which letter would you like to replace: ")
                hand = substitute_hand(hand, sub_letter)
                # Display new hand with substitute
        curr_hand_score = play_hand(hand, word_list)
        # Option to replay hand
        while replay:
            user_replay = input("Would you like to replay the hand? ")
            # If user doesn't want to replay, break out
            if user_replay.lower() == 'no' or user_replay.lower() == 'nope' or user_replay.lower() == 'n':
                replay = False
            else:
                curr_hand_score = play_hand(hand, word_list)
        # Add score of hand to scores list
        scores.append(curr_hand_score)
        #Reduce hand number by 1 each time
        hand_number -= 1
    # Display total score for hands played
    print("----------")
    for score in scores:
        total_score += score
    print("Total score over all hands: ", total_score)
    # Display best hand number
    for i in range(len(scores)):
        if scores[i] > best_hand:
            best_hand = scores[i]+1
    print("Best hand played was hand", best_hand)
#
